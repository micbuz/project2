...
def to_ogorki(tekst):
    ...
    tekst=buf.readlines()
    #with open('tekst_wynikowy.txt', 'w') as w:

    poczatek=0 #this show from which line to cut previous string
    koniec=[]
    for i, item in enumerate(tekst):

        row = item.replace('\n','').split(' ') 
        #we try to use levenstein to find out in which row 'fiskalny' is
        for j in row:
    ...
    #I remow first element from list if it is new line or it is equal to 1
    ...
    tekst3=[]
    #for i,item  in enumerate(tekst2):
     #   if tekst2[i+1] != '\n':
      #      tekst3.append(item)
       # else: 
    ...
    #/\mozliwe ze ten krok nie jest potrzebny bo i tak trzeba to bedzie wrzucic do bazy danych
